 ...
 */
export default function WhatsappDocumenti() {
  ...
  return (
    <div className="mt-4 rounded-2xl bg-neutral-50 p-5">
      <button
        ...
        className="premi flex w-full items-center justify-between text-left"
      >
        <span className="font-semibold text-neutral-900">
          Reparto fascicoli — allegati degli assistiti
          {documenti.length > 0 && (
        ...
        <span className="text-xs text-neutral-500">{aperto ? 'Nascondi' : 'Mostra'}</span>
      </button>

      {aperto && (
        gruppi.length === 0 ? (
          <p className="mt-3 text-sm text-neutral-500">Ancora nessun documento ricevuto su WhatsApp.</p>
        ) : (
          <div className="mt-4 space-y-4">
            {gruppi.map((g) => (
              <div key={g.titolo} className="rounded-xl bg-white p-4">
                <p className="mb-2 text-sm font-medium text-neutral-900">{g.titolo}</p>
                <ul className="space-y-1.5">
                  {g.documenti.map((d) => (
                    <li key={d.id} className="flex flex-wrap items-center gap-2 text-xs">
                      ...
                      <span className="text-neutral-400">{dataIt(d.ricevutoIl)}</span>
                      {d.praticaLabel ? (
