...
type Proposta = {
  ...
  whatsapp_messaggi?: { jid_mittente: string | null; ricevuto_il: string | null };
};
...
/**
 * Le scadenze che Themis ha trovato nei messaggi WhatsApp, in attesa di
 * una decisione. Stessa logica di ScadenzeProposte.tsx (PEC): nessuna
 * finisce in calendario da sola, ogni proposta mostra la frase esatta da
 * cui viene.
 */
export default function WhatsappProposte() {
  ...
  const carica = useCallback(async () => {
    const res = await fetch('/api/whatsapp/proposte');
    if (!res.ok) return;
  ...
  async function analizza() {
    setAnalisi(true);
    setMessaggio('');
    const res = await fetch('/api/themis/whatsapp-scadenze', { method: 'POST' });
    const body = await res.json();
    setAnalisi(false);
    if (!res.ok) { setMessaggio(body.error || 'Analisi non riuscita'); return; }
    setMessaggio(body.esaminati === 0 && !body.falliti
      ? 'Nessun messaggio nuovo da esaminare.'
      : `${body.esaminati} messaggi letti, ${body.proposte} scadenze trovate.`
        + (body.falliti ? ` ${body.falliti} non analizzati: riprova fra un momento.` : '')
        + (body.restanti > 0 ? ` Altri ${body.restanti} da esaminare: premi di nuovo.` : ''));
    carica();
  }

  async function decidi(id: string, azione: 'accetta' | 'rifiuta') {
    setInCorso(id);
    await fetch('/api/whatsapp/proposte', {
          ...
          <div className="min-w-0">
            ...
            {messaggio ? (
              ...
              <p className="mt-0.5 text-sm text-neutral-500">
                ...
                i messaggi non ancora esaminati.
              </p>
            ) : (
              <p className="mt-0.5 text-sm text-neutral-500">
