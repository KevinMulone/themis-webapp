...
export const Select = forwardRef<HTMLSelectElement, React.SelectHTMLAttributes<HTMLSelectElement> & {
  ...
  return (
    <div className={full ? 'col-span-full' : ''}>
      ...
      <select
        ref={ref}
        className={`w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm outline-none transition-colors focus:border-bordeaux-400 focus:bg-white ${className}`.trim()}
        {...props}
      >
        {children}
      </select>
